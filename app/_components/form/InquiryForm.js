 ...
 */
export default function InquiryForm({show}) {
  const companyNameInput = useRef(null)

  useEffect(() => {
    if (show) {
      // モーダル表示時に企業名欄にフォーカスする
  ...
  return (
      <div className="px-3 py-4">
        <h3 className="mb-2 text-base md:text-xl text-white text-center">問い合わせ</h3>
        <form className="space-y-4" action={`mailto: ${process.env.NEXT_PUBLIC_INQUIRY_MAIL}?subject= ${process.env.NEXT_PUBLIC_INQUIRY_MAIL_SUBJECT}`} method="POST">
          <div>
            <label
